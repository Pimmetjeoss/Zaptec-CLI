...
Exit codes & warnings:
  ...
  zaptec-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zaptec-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zaptec-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zaptec-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zaptec-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zaptec-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "chargehistory":
		return db.UpsertChargehistory(data)
	case "charger_firmware":
		return db.UpsertChargerFirmware(data)
	case "chargers":
		return db.UpsertChargers(data)
	case "send_command":
		return db.UpsertSendCommand(data)
	case "state":
		return db.UpsertState(data)
	case "chargers_update":
		return db.UpsertChargersUpdate(data)
	case "constants":
	...
	case "installation_messaging_connection_details":
		return db.UpsertInstallationMessagingConnectionDetails(data)
	case "installation_update":
		return db.UpsertInstallationUpdate(data)
	case "session":
		return db.UpsertSession(data)
	case "priority":
		return db.UpsertPriority(data)
	case "user_groups_messaging_connection_details":
		return db.UpsertUserGroupsMessagingConnectionDetails(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		"chargehistory",
		"chargehistory-installationreport",
		"chargers",
		"installation",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"chargehistory-installationreport": "/api/chargehistory/installationreport",
