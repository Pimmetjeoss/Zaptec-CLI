...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zaptec-pp-cli",
		Short: `Zaptec CLI — The only command-line tool for Zaptec Go chargers — pause and resume charging, read live state in plain English, and tr…`,
		Long: `Zaptec CLI — The only command-line tool for Zaptec Go chargers — pause and resume charging, read live state in plain English, and tr…
...
Health check: run 'zaptec-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zaptec-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zaptec-pp-cli profile list')")
	...
	rootCmd.AddCommand(newChargehistoryCmd(flags))
	chargersCmd := newChargersCmd(flags)
	chargersCmd.AddCommand(newChargersStaleCmd(flags))
	rootCmd.AddCommand(chargersCmd)
	rootCmd.AddCommand(newInstallationCmd(flags))
	...
	// Hand-built friendly control commands (sendCommand wrappers).
	rootCmd.AddCommand(newPauseCmd(flags))
	rootCmd.AddCommand(newResumeCmd(flags))
	rootCmd.AddCommand(newStartCmd(flags))
	rootCmd.AddCommand(newStopCmd(flags))
	rootCmd.AddCommand(newRestartCmd(flags))
	rootCmd.AddCommand(newUnlockCmd(flags))
	rootCmd.AddCommand(newDeauthorizeCmd(flags))

	// Hand-built telemetry + analytics (transcendence) commands.
	rootCmd.AddCommand(newStateCmd(flags))
	rootCmd.AddCommand(newLiveCmd(flags))
	rootCmd.AddCommand(newCostCmd(flags))
	rootCmd.AddCommand(newSessionsCmd(flags))
	rootCmd.AddCommand(newFirmwareCmd(flags))
	...
	rootCmd.AddCommand(newChargerFirmwarePromotedCmd(flags))
	rootCmd.AddCommand(newConstantsPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// Transparent OAuth2 password-grant login when ZAPTEC_USERNAME/PASSWORD are
	// set and no usable token is cached. Best-effort; no-op under verify/dry-run.
	maybeAutoLogin(f, cfg)
