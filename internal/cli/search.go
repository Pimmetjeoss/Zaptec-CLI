...
In local mode: searches locally synced data only.`,
  ...
  zaptec-pp-cli search "error timeout"

  # Force local search only
  zaptec-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  zaptec-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  zaptec-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'zaptec-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "installation":
				results, err = db.SearchInstallation(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchInstallation(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search installation failed: %w", searchErr)
					}
