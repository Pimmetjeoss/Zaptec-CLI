...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zaptec-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zaptec-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"chargehistory", "chargehistory-installationreport", "chargers", "installation"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zaptec-pp-cli workflow status

  # Show status as JSON
  zaptec-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zaptec-pp-cli")
			}
